...
const Dashboard = (() => {
  ...
  // Fallback poll timer for when WebSocket drops
  ...
  function startProgressTracking(taskId) {
    ...
    activeProgressWs = API.createProgressWebSocket(taskId, {
      onOpen() {
        ...
        // Start client-side ping every 20s to keep connection alive
        if (wsPingTimer) clearInterval(wsPingTimer);
        wsPingTimer = setInterval(() => {
          if (activeProgressWs) activeProgressWs.send('ping');
        }, 20000);
      },
      onMessage(data) {
        // Skip ping/pong control messages
        if (data.event === 'ping' || data.event === 'pong') return;
    ...
    // Start fallback HTTP polling every 10s
  ...
  function startProgressPolling(taskId) {
    ...
    progressPollTimer = setInterval(async () => {
      try {
        const task = await API.getVideoTask(taskId);
        if (!task) return;
        // Build a synthetic WS-like message from the HTTP response
        const syntheticData = {
          task_id: taskId,
          event: 'poll',
          status: task.status,
          progress_segment: task.progress_segment || 0,
          progress_total: task.progress_total || 0,
          error_message: task.error_message || null,
          result_filename: task.result_filename || null,
          drive_link: task.result_drive_link || null,
        };
        handleProgressMessage(taskId, syntheticData);
  ...
  function handleProgressMessage(taskId, data) {
    ...
    if (data.status === 'processing') {
      ...
      } else if (segLabel && segLabel.textContent === '等待开始') {
        segLabel.textContent = '处理中...';
      }
      ...
      } else if (!data.stage && data.progress_segment > 0) {
        // From HTTP poll: no stage info, but we have segment progress → assume generating_video
        const videoIdx = PIPELINE_STAGES.findIndex(s => s.key === 'generating_video');
        PIPELINE_STAGES.forEach((s, i) => {
          if (i < videoIdx)      updatePipelineStage(s.key, 'done');
          else if (i === videoIdx) updatePipelineStage(s.key, 'active', `片段 ${data.progress_segment}/${data.progress_total}`);
          else                    updatePipelineStage(s.key, 'pending', '等待中');
        });
      } else if (!data.stage && segLabel.textContent === '等待开始') {
        // Generic processing, show first stage as active
        updatePipelineStage(PIPELINE_STAGES[0].key, 'active', '处理中...');
      }
