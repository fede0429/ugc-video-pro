...
let _rightsManifestFile = null;
let _rightsManifestSummary = null;
...
function formatComplianceSummary(summary) {
  if (!summary) return '等待上传 rights_manifest.json 后显示授权摘要。';
  const lines = [
    `授权主体：${summary.rights_owner || '-'}`,
    `授权类型：${summary.subject_type || '-'}`,
    `有效期：${summary.valid_from || '-'} → ${summary.valid_until || '-'}`,
    `语言：${(summary.languages || []).join(' / ') || '-'}`,
    `平台：${(summary.platforms || []).join(' / ') || '-'}`,
    `允许头像口播：${summary.allow_lipsync ? '是' : '否'}`,
    `允许视频驱动：${summary.allow_face_reenactment ? '是' : '否'}`,
    `图片哈希：${summary.bound_assets?.presenter_image_sha256 ? summary.bound_assets.presenter_image_sha256.slice(0, 12) + '…' : '-'}`,
    `视频哈希：${summary.bound_assets?.presenter_video_sha256 ? summary.bound_assets.presenter_video_sha256.slice(0, 12) + '…' : '-'}`,
  ];
  return lines.join('\n');
}

async function parseRightsManifest(file) {
  ...
  const summary = {
    ...
    languages: data.usage_scope?.languages || [],
    platforms: data.usage_scope?.platforms || data.usage_scope?.channels || [],
    allow_lipsync: !!data.usage_scope?.allow_lipsync,
    allow_face_reenactment: !!data.usage_scope?.allow_face_reenactment,
    bound_assets: data.bound_assets || {},
  };
  _rightsManifestSummary = summary;
  const summaryEl = document.getElementById('rights-summary');
  if (summaryEl) summaryEl.textContent = formatComplianceSummary(summary);
}

document.getElementById('rights-input')?.addEventListener('change', async e => {
  const file = e.target.files?.[0] || null;
  _rightsManifestFile = file;
  document.getElementById('rights-name').textContent = file ? file.name : '未选择文件';
  try {
    if (file) await parseRightsManifest(file);
    else {
      _rightsManifestSummary = null;
      document.getElementById('rights-summary').textContent = formatComplianceSummary(null);
    }
  } catch (err) {
    _rightsManifestSummary = null;
    document.getElementById('rights-summary').textContent = 'rights_manifest.json 解析失败：' + err.message;
    showToast('rights_manifest.json 解析失败，请检查 JSON 格式', 'error');
  }
});
...
document.getElementById('ugc-form')?.addEventListener('submit', async e => {
  ...
  try {
    ...
    const hasPresenter = !!(_presenterImage || _presenterVideo);
    if (hasPresenter) {
      if (!_rightsManifestFile) throw new Error('使用真人头像或主播短视频时，必须上传 rights_manifest.json');
      if (!document.getElementById('consent-confirmed').checked) throw new Error('请先勾选“我确认已获得合法授权”');
      fd.append('rights_manifest', _rightsManifestFile);
      fd.append('consent_confirmed', 'true');
    } else {
      fd.append('consent_confirmed', document.getElementById('consent-confirmed').checked ? 'true' : 'false');
    }
    ...
    fd.append('voice_preset',  _chipValues.language);
...
function renderTaskList(tasks) {
  ...
  el.innerHTML = tasks.map(t => {
    const compliance = t.qa_report_json?.compliance || null;
    const summary = compliance?.summary || {};
    const complianceText = compliance ? [
      `状态：${compliance.status || 'approved'}`,
      `到期：${summary.valid_until || '-'}`,
      `合规单号：${compliance.compliance_token || '-'}`,
      `图片哈希：${summary.bound_assets?.presenter_image_sha256 ? summary.bound_assets.presenter_image_sha256.slice(0,12) + '…' : '-'}`,
      `视频哈希：${summary.bound_assets?.presenter_video_sha256 ? summary.bound_assets.presenter_video_sha256.slice(0,12) + '…' : '-'}`,
    ].join(' ｜ ') : '';
    return `
    <div class="task-row ${t.id === _activeTaskId ? 'task-row--active' : ''}" data-id="${t.id}">
      ...
      <div class="task-info">
        ...
        ${complianceText ? `<div class="task-compliance">🧾 ${complianceText}</div>` : ''}
      </div>
      <div class="task-actions">
        ${t.status === 'completed' ? `<a href="${API}/video/download/${t.id}" class="btn btn-ghost btn-xs" download>⬇ 下载</a>` : ''}
        <button class="btn btn-ghost btn-xs" onclick="resumeTask('${t.id}')">👁 查看</button>
        <button class="btn btn-ghost btn-xs" onclick="deleteTask('${t.id}')">🗑 删除</button>
      </div>
    </div>`;
  }).join('');
}
