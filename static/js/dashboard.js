...
const Dashboard = (() => {
  ...
  // Must match models/base.py MODEL_MAX_DURATIONS and kie_video.py max_seconds
  const MODEL_MAX_DURATIONS = {
    veo_31_fast:    8,
    veo_31_quality: 8,
    seedance_15:   12,   // valid: 4, 8, 12
    seedance_2:    12,   // valid: 4, 5, 8, 10, 12
    sora_2:        10,
    runway:         5,   // runway-duration-5-generate
    kling_30:      15,   // 3-15s range
    hailuo:         6,
  };
  ...
  function showToast(type, title, message) {
    ...
    toast.innerHTML = `<div class="toast-icon">${icons[type] || icons.info}</div><div class="toast-body"><div class="toast-title">${escapeHtml(title)}</div>${message ? `<div class="toast-message">${escapeHtml(message)}</div>` : ''}</div>`;
  ...
  function activateSection(sectionName) {
    ...
    if (sectionName === 'my-videos') loadVideoTasks(1);
    else if (sectionName === 'settings') loadSettings();
    else if (sectionName === 'admin-users') Admin.loadUsers();
    else if (sectionName === 'admin-invites') Admin.loadInviteCodes();
    closeMobileSidebar();
  }
  ...
  function initSidebar() {
    ...
    if (hamburger) { hamburger.addEventListener('click', () => { sidebar.classList.add('mobile-open'); backdrop.classList.add('visible'); }); }
    if (backdrop) { backdrop.addEventListener('click', closeMobileSidebar); }
    document.querySelectorAll('.nav-item[data-section]').forEach(item => {
      item.addEventListener('click', () => navigate(item.dataset.section));
      item.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(item.dataset.section); } });
    });
  ...
  async function loadUserProfile() {
    try {
      ...
      if (currentUser.role === 'admin') { document.querySelectorAll('.admin-only').forEach(el => { el.style.display = ''; }); }
  ...
  function renderStepsHeader() {
    ...
    STEP_LABELS.forEach((label, idx) => {
      ...
      dot.innerHTML = `<div class="step-dot-circle">${stepNum < currentStep ? '✓' : stepNum}</div><div class="step-dot-label">${label}</div>`;
  ...
  function updateStepUI() {
    ...
    if (progressArea) { progressArea.style.display = currentStep === 4 ? 'none' : ''; }
  }
  ...
  function initImageUploadGrid() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      slot.innerHTML = `${i === 0 ? '<span class="slot-label">主图</span>' : ''}<input type="file" accept="image/jpeg,image/png,image/webp" style="display:none;" id="slotInput${i}"><img src="" alt="预览" id="slotImg${i}"><span class="slot-add-icon">${i === 0 ? '🖼' : '＋'}</span><span class="slot-add-text" style="font-size:10px;color:var(--text-muted);">${i === 0 ? '上传主图' : '添加图片'}</span><button class="slot-remove" id="slotRemove${i}" title="移除图片" tabindex="-1"><svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M1 1l8 8M9 1L1 9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg></button>`;
  ...
  function bindSlot(slot, idx) {
    ...
    slot.addEventListener('click', (e) => { if (e.target === removeBtn || removeBtn.contains(e.target)) return; input.click(); });
    slot.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); input.click(); } });
    slot.addEventListener('dragover', (e) => { e.preventDefault(); slot.classList.add('drag-over'); });
    slot.addEventListener('dragleave', () => slot.classList.remove('drag-over'));
    slot.addEventListener('drop', (e) => { e.preventDefault(); slot.classList.remove('drag-over'); const file = e.dataTransfer.files[0]; if (file) handleSlotFile(idx, file, img, slot); });
    input.addEventListener('change', (e) => { const file = e.target.files[0]; if (file) handleSlotFile(idx, file, img, slot); input.value = ''; });
    removeBtn.addEventListener('click', (e) => { e.stopPropagation(); clearSlot(idx, img, slot); });
  }

  function handleSlotFile(idx, file, img, slot) {
    if (!file.type.startsWith('image/')) { showToast('error', '文件类型错误', '请上传 JPG、PNG 或 WebP 图片'); return; }
    if (file.size > 20 * 1024 * 1024) { showToast('error', '图片过大', '最大支持 20MB'); return; }
    wizardData.images[idx] = file;
    const reader = new FileReader();
    reader.onload = (e) => { img.src = e.target.result; slot.classList.add('has-file'); };
  ...
  function bindStep1() {
    ...
    document.getElementById('step1Next').addEventListener('click', () => {
      const hasImages = wizardData.images.filter(Boolean).length > 0;
      const hasText   = (document.getElementById('productText')?.value?.trim()?.length || 0) > 5;
      if (hasImages || hasText) { showStep(2); }
    });
  }
  ...
  function bindStep2() {
    ...
    document.getElementById('applyCustomDuration')?.addEventListener('click', () => {
      ...
      if (!val || val < 1) { errEl.textContent = '请输入有效的秒数（最少1秒）'; errEl.classList.add('visible'); return; }
    ...
    document.getElementById('customDuration')?.addEventListener('keydown', (e) => { if (e.key === 'Enter') document.getElementById('applyCustomDuration').click(); });
    document.getElementById('customDuration')?.addEventListener('input', () => { const errEl = document.getElementById('customDurationError'); if (errEl) errEl.classList.remove('visible'); });
    document.querySelectorAll('input[name="lang"]').forEach(cb => {
      cb.addEventListener('change', () => {
        ...
        if (checked.length === 0) { document.getElementById('langIt').checked = true; wizardData.languages = ['it']; }
        checkStep2Valid();
      });
    });
    ...
    if (advancedToggleBtn && advancedWrap) {
      advancedToggleBtn.addEventListener('click', () => {
        ...
        advancedToggleBtn.innerHTML = `<svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2 4l4 4 4-4" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg> ${isOpen ? '收起模型列表' : '展开全部模型（高级选项）'}`;
      });
    }
    ...
    document.getElementById('step2Next').addEventListener('click', () => { if (checkStep2Valid()) { buildSummary(); showStep(3); } });
  }
  ...
  function buildSummary() {
    ...
    if (imageCount > 0) rows.push({ label: '图片', value: `${imageCount} 张（主图: ${wizardData.images[0]?.name || '未知'}）` });
    if (wizardData.productUrl) rows.push({ label: '链接', value: wizardData.productUrl.slice(0, 60) + (wizardData.productUrl.length > 60 ? '…' : '') });
    if (wizardData.textPrompt) rows.push({ label: '描述', value: wizardData.textPrompt.slice(0, 80) + (wizardData.textPrompt.length > 80 ? '…' : '') });
    const tierPriceMap = { economy: '$0.04', premium: '$1.25', china: '特价' };
    const pricePerClip = tierKey ? tierPriceMap[tierKey] : '—';
    if (pricePerClip && pricePerClip !== '—') rows.push({ label: '预估费用', value: `约 ${pricePerClip}/片段 × ${segments} 片段` });
    grid.innerHTML = rows.map(r => `<div class="summary-row"><span class="summary-label">${escapeHtml(r.label)}</span><span class="summary-value">${escapeHtml(r.value)}</span></div>`).join('');
  }
  ...
  function buildFormData() {
    ...
    fd.append('language', wizardData.languages.join(','));
  ...
  function resetWizard() {
    ...
    document.querySelectorAll('input[name="lang"]').forEach(cb => { cb.checked = cb.value === 'it'; });
    document.querySelectorAll('.duration-btn').forEach(b => b.classList.remove('selected'));
    ...
    if (startBtn) { startBtn.disabled = false; startBtn.innerHTML = `<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M5 3l9 5-9 5V3Z" fill="currentColor"/></svg> 开始生成`; }
    const indicator = document.getElementById('autoModeIndicator');
    ...
    if (advBtn) { advBtn.classList.remove('open'); advBtn.innerHTML = `<svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2 4l4 4 4-4" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg> 展开全部模型（高级选项）`; }
    if (activeProgressWs) { activeProgressWs.close(); activeProgressWs = null; }
  ...
  function renderPipelineStages() {
    ...
    PIPELINE_STAGES.forEach((stage, idx) => {
      ...
      div.innerHTML = `<div class="pipeline-stage-left"><div class="pipeline-stage-dot" id="pipelineDot_${stage.key}"><span>${stage.icon}</span></div><div class="pipeline-stage-line"></div></div><div class="pipeline-stage-content"><div class="pipeline-stage-label">${stage.label}</div><div class="pipeline-stage-status" id="pipelineStatus_${stage.key}">等待中</div></div>`;
  ...
  function resetPipelineStages() {
    ...
    document.getElementById('progressCompleteBox')?.style && (document.getElementById('progressCompleteBox').style.display = 'none');
    document.getElementById('progressFailedBox')?.style && (document.getElementById('progressFailedBox').style.display = 'none');
  ...
  function startProgressTracking(taskId) {
    ...
    activeProgressWs = API.createProgressWebSocket(taskId, {
      onMessage(data) { handleProgressMessage(taskId, data); },
      onError() {},
      onClose() { activeProgressWs = null; },
    });
  }
  ...
  function handleProgressMessage(taskId, data) {
    ...
    if (data.status === 'processing') {
      ...
      if (dlBtn) { dlBtn.style.display = 'inline-flex'; dlBtn.onclick = () => downloadTask(taskId); }
      if (activeProgressWs) { activeProgressWs.close(); activeProgressWs = null; }
  ...
  function bindProgressView() {
    document.getElementById('progressCancelBtn')?.addEventListener('click', () => { if (activeProgressWs) { activeProgressWs.close(); activeProgressWs = null; } resetWizard(); });
    document.getElementById('progressGoToVideosBtn')?.addEventListener('click', () => { navigate('my-videos'); });
    document.getElementById('progressGoToVideosBtn2')?.addEventListener('click', () => { navigate('my-videos'); });
    document.getElementById('progressRetryBtn')?.addEventListener('click', () => { resetWizard(); showStep(1); });
    const newTaskBtn = document.getElementById('progressNewTaskBtn');
    if (newTaskBtn) { newTaskBtn.addEventListener('click', () => { resetWizard(); showStep(1); }); }
  }
  ...
  async function loadVideoTasks(page = 1) {
    ...
    try {
      ...
      if (tasks.length === 0) { if (emptyEl) emptyEl.style.display = 'flex'; return; }
      if (videosViewMode === 'list') renderTasksTable(tasks);
      else renderTasksGrid(tasks);
      renderPagination();
      tasks.forEach(task => { if (['pending', 'processing'].includes(task.status)) subscribeTaskProgress(task.id); });
    } catch (err) { if (loadingEl) loadingEl.style.display = 'none'; showToast('error', '加载失败', err.message); }
  }
  ...
  function renderTasksTable(tasks) {
    ...
    tasks.forEach(task => {
      ...
      tr.innerHTML = `<td><span class="badge ${status.cls}" id="statusBadge_${task.id}">${status.text}</span><div class="inline-progress" id="inlineProgress_${task.id}" style="display:none;margin-top:4px;"><svg width="12" height="12" viewBox="0 0 12 12"><circle cx="6" cy="6" r="5" stroke="#3b82f6" stroke-width="1.5" stroke-dasharray="31.4" stroke-dashoffset="15.7" fill="none"><animateTransform attributeName="transform" type="rotate" dur="1s" repeatCount="indefinite" from="0 6 6" to="360 6 6"/></circle></svg><span id="progressText_${task.id}"></span></div></td><td>${escapeHtml(MODE_DISPLAY_NAMES[task.mode] || task.mode)}</td><td>${escapeHtml(MODEL_DISPLAY_NAMES[task.model] || task.model)}</td><td>${task.duration ? task.duration + 's' : '—'}</td><td>${escapeHtml(LANG_DISPLAY_NAMES[task.language] || task.language || '—')}</td><td style="white-space:nowrap">${formatDate(task.created_at)}</td><td><div class="table-actions">${task.status === 'completed' ? `<button class="btn btn-success btn-sm" onclick="Dashboard.downloadTask('${task.id}')"><svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M6 1v7M3 5l3 3 3-3M1 10h10" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/></svg>下载</button>` : ''}${task.status === 'failed' ? `<span class="text-danger text-xs" style="max-width:120px;overflow:hidden;text-overflow:ellipsis;" title="${escapeHtml(task.error_message || '')}">${escapeHtml((task.error_message || '').slice(0, 30))}</span>` : ''}<button class="btn btn-ghost btn-icon btn-sm" onclick="Dashboard.confirmDelete('${task.id}')" title="删除"><svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2 3h8M4 3V2h4v1M5 5v4M7 5v4M3 3l.6 7h4.8L9 3" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg></button></div></td>`;
  ...
  function renderTasksGrid(tasks) {
    ...
    tasks.forEach(task => {
      ...
      card.innerHTML = `<div class="task-header"><div class="task-meta"><div class="task-mode">${escapeHtml(MODE_DISPLAY_NAMES[task.mode] || task.mode)}</div><div class="task-model">${escapeHtml(MODEL_DISPLAY_NAMES[task.model] || task.model)}</div></div><span class="badge ${status.cls}" id="cardStatus_${task.id}">${status.text}</span></div><div class="task-body"><div class="task-info-row"><div class="task-info-item"><svg width="12" height="12" viewBox="0 0 12 12" fill="none"><circle cx="6" cy="6" r="5" stroke="currentColor" stroke-width="1.2"/><path d="M6 3v3l2 2" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>${task.duration ? task.duration + '秒' : '—'}</div><div class="task-info-item"><svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M6 1l1.5 3 3.5.5-2.5 2.5.5 3.5L6 9l-3 1.5.5-3.5L1 4.5l3.5-.5z" stroke="currentColor" stroke-width="1.1" stroke-linejoin="round"/></svg>${escapeHtml(LANG_DISPLAY_NAMES[task.language] || task.language || '—')}</div></div><div class="task-progress" id="cardProgress_${task.id}"><div class="task-progress-bar-wrap"><div class="task-progress-bar" id="cardProgressBar_${task.id}" style="width:0%"></div></div><div class="task-progress-text" id="cardProgressText_${task.id}"></div></div><div class="task-error" id="cardError_${task.id}">${escapeHtml(task.error_message || '')}</div></div><div class="task-footer"><span class="task-date">${formatDate(task.created_at)}</span><div style="display:flex;gap:var(--sp-2);">${task.status === 'completed' ? `<button class="btn btn-success btn-sm" onclick="Dashboard.downloadTask('${task.id}')">下载</button>` : ''}<button class="btn btn-ghost btn-sm btn-icon" onclick="Dashboard.confirmDelete('${task.id}')" title="删除"><svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2 3h8M4 3V2h4v1M5 5v4M7 5v4M3 3l.6 7h4.8L9 3" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg></button></div></div>`;
  ...
  function updateTaskStatus(taskId, data) {
    ...
    if (statusBadge) { statusBadge.textContent = status.text; statusBadge.className = `badge ${status.cls}`; }
    if (cardStatus) { cardStatus.textContent = status.text; cardStatus.className = `badge ${status.cls}`; }
    if (data.status === 'processing') {
      const pct = data.progress_total > 0 ? Math.round((data.progress_segment / data.progress_total) * 100) : 0;
      ...
      if (data.status === 'failed' && data.error_message) { if (cardError) { cardError.textContent = data.error_message; cardError.classList.add('visible'); } }
      if (data.status === 'completed' || data.status === 'failed') {
        if (activeWebSockets[taskId]) { activeWebSockets[taskId].close(); delete activeWebSockets[taskId]; }
  ...
  function subscribeTaskProgress(taskId) {
    ...
    const ws = API.createProgressWebSocket(taskId, {
      onMessage(data) { updateTaskStatus(taskId, data); },
      onError() {},
      onClose() { delete activeWebSockets[taskId]; },
    });
  ...
  async function downloadTask(taskId) {
    ...
    } catch (err) { showToast('error', '下载失败', err.message); }
  }
  ...
  async function executeDelete() {
    ...
    try { await API.deleteVideoTask(id); showToast('success', '已删除', '视频任务已删除'); loadVideoTasks(videoTasksPage); }
  ...
  function initSettings() {
    ...
    savePwdBtn.addEventListener('click', async () => {
      ...
      if (!current || !newPwd || !confirm) { showToast('warning', '请填写所有字段', ''); return; }
      if (newPwd.length < 8) { showToast('warning', '新密码至少8位', ''); return; }
      if (newPwd !== confirm) { showToast('warning', '两次密码不一致', ''); return; }
      ...
      } catch (err) { showToast('error', '修改失败', err.message); }
      finally { savePwdBtn.disabled = false; }
    });
  }
  ...
  function initViewToggle() {
    document.querySelectorAll('[data-view]').forEach(tab => {
      tab.addEventListener('click', () => {
        ...
        if (videosViewMode === 'list') { if (listView) listView.style.display = 'block'; if (gridView) gridView.style.display = 'none'; }
        else { if (listView) listView.style.display = 'none'; if (gridView) gridView.style.display = 'block'; }
        loadVideoTasks(1);
      });
    });
  }
  ...
  function initModals() {
    document.getElementById('confirmDeleteClose')?.addEventListener('click', () => { document.getElementById('confirmDeleteModal').classList.remove('open'); pendingDeleteId = null; });
    document.getElementById('confirmDeleteCancel')?.addEventListener('click', () => { document.getElementById('confirmDeleteModal').classList.remove('open'); pendingDeleteId = null; });
    document.getElementById('confirmDeleteOk')?.addEventListener('click', executeDelete);
    document.querySelectorAll('.modal-overlay').forEach(overlay => {
      overlay.addEventListener('click', (e) => { if (e.target === overlay) { overlay.classList.remove('open'); pendingDeleteId = null; } });
    });
  }
  ...
  function startAutoRefresh() {
    videoRefreshTimer = setInterval(() => {
      ...
      if (myVideosSection?.classList.contains('active')) loadVideoTasks(videoTasksPage);
    }, 30000);
  }
  ...
  function handleHashChange() {
    ...
    else activateSection('new-video');
  }
  ...
  async function init() {
    ...
    initModals();
    startAutoRefresh();
    initActiveTasksPanel();
    refreshActiveTasksList();
    window.addEventListener('hashchange', handleHashChange);
    handleHashChange();
  }

  function initActiveTasksPanel() {
    const header = document.getElementById('activeTasksPanelHeader');
    if (header) {
      header.addEventListener('click', () => {
        const panel = document.getElementById('activeTasksPanel');
        if (panel) { tasksPanelCollapsed = !tasksPanelCollapsed; panel.classList.toggle('collapsed', tasksPanelCollapsed); }
      });
    }
    setInterval(refreshActiveTasksList, 5000);
  }

  async function refreshActiveTasksList() {
    try {
      const data = await API.request('/api/tasks?status=pending,processing&page_size=10');
      const tasks = data.items || [];
      const currentIds = new Set(tasks.map(t => t.id));
      for (const tid of Object.keys(activeTasks)) {
        if (!currentIds.has(tid)) {
          if (activeTasks[tid].ws) { try { activeTasks[tid].ws.close(); } catch(e) {} }
          delete activeTasks[tid];
        }
      }
      for (const t of tasks) {
        if (!activeTasks[t.id]) {
          activeTasks[t.id] = { model: t.model, duration: t.duration, status: t.status, stage: '', stage_message: '', progress_segment: t.progress_segment || 0, progress_total: t.progress_total || 0, ws: null };
          connectTaskWebSocket(t.id);
        } else {
          activeTasks[t.id].status = t.status;
          activeTasks[t.id].progress_segment = t.progress_segment || activeTasks[t.id].progress_segment;
          activeTasks[t.id].progress_total = t.progress_total || activeTasks[t.id].progress_total;
        }
      }
  ...
  function connectTaskWebSocket(taskId) {
    if (activeTasks[taskId]?.ws) return;
    const proto = location.protocol === 'https:' ? 'wss' : 'ws';
    const wsUrl = `${proto}://${location.host}/ws/${taskId}`;
    try {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (activeTasks[taskId]) {
            if (msg.status) activeTasks[taskId].status = msg.status;
            if (msg.stage) activeTasks[taskId].stage = msg.stage;
            if (msg.stage_message) activeTasks[taskId].stage_message = msg.stage_message;
            if (msg.progress_segment !== undefined) activeTasks[taskId].progress_segment = msg.progress_segment;
            if (msg.progress_total !== undefined) activeTasks[taskId].progress_total = msg.progress_total;
            if (msg.status === 'completed' || msg.status === 'failed') {
              setTimeout(() => {
                delete activeTasks[taskId];
                renderActiveTasksPanel();
                if (document.getElementById('section-my-videos')?.classList.contains('active')) loadVideoTasks(1);
              }, 3000);
            }
            renderActiveTasksPanel();
          }
      ...
      ws.onerror = () => {};
      ws.onclose = () => { if (activeTasks[taskId]) activeTasks[taskId].ws = null; };
  ...
  function renderActiveTasksPanel() {
    ...
    const statusLabels = { pending: '等待中', processing: '生成中', completed: '已完成', failed: '失败' };
    body.innerHTML = taskIds.map(tid => {
      const t = activeTasks[tid];
      const model = MODEL_DISPLAY_NAMES[t.model] || t.model;
      const statusText = statusLabels[t.status] || t.status;
      const pct = t.progress_total > 0 ? Math.round((t.progress_segment / t.progress_total) * 100) : (t.status === 'processing' ? 10 : 0);
      const info = t.stage_message || `${t.duration}秒 · ${model}`;
      const shortId = tid.slice(0, 8);
      return `<div class="task-mini-card" onclick="Dashboard.showTaskProgress('${tid}')"><div class="task-mini-card-top"><span class="task-mini-model">${escapeHtml(model)} · ${t.duration}s</span><span class="task-mini-status ${t.status}">${statusText}</span></div><div class="task-mini-bar-wrap"><div class="task-mini-bar" style="width:${pct}%"></div></div><div class="task-mini-info">${escapeHtml(info)} · #${shortId}</div></div>`;
    }).join('');
  }

  function showTaskProgressFromPanel(taskId) {
    activeProgressTaskId = taskId;
    startProgressTracking(taskId);
    showProgressView(taskId);
    navigate('new-video');
  }
  ...
  return {
    ...
    confirmDelete,
    showToast,
    showTaskProgress: showTaskProgressFromPanel,
  };
})();
