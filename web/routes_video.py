...
import asyncio, json, math, os, shutil, uuid
...
from web.compliance import compute_sha256_for_path, parse_manifest_bytes, validate_rights_manifest
...
def _normalize_quality_tier(value: str) -> str:
    ...
    return mapping.get((value or "").strip(), "premium")


def _build_compliance_payload(
    *,
    validation_result,
    manifest_original: dict[str, Any],
    rights_manifest_filename: str,
    presenter_image_sha256: str | None,
    presenter_video_sha256: str | None,
) -> dict[str, Any]:
    return {
        "status": validation_result.summary.get("status", "approved"),
        "mode": "rights_manifest_json",
        "rights_manifest_filename": rights_manifest_filename,
        "rights_manifest_original": manifest_original,
        "rights_manifest_normalized": validation_result.normalized_manifest,
        "summary": validation_result.summary,
        "compliance_token": validation_result.compliance_token,
        "bound_assets": {
            "presenter_image_sha256": presenter_image_sha256,
            "presenter_video_sha256": presenter_video_sha256,
        },
    }
...
async def generate_ugc_video(
    ...
    rights_manifest: Optional[UploadFile] = File(default=None, description="rights_manifest.json"),
    # ── Text / JSON params ────────────────────────────────────────────────────
    ...
    presenter_image_sha256: Optional[str] = None
    if presenter_image:
        if presenter_image.content_type not in ALLOWED_IMAGE_TYPES:
            raise HTTPException(status_code=400, detail=f"Invalid presenter image type: {presenter_image.content_type}")
        presenter_fname = await _save_upload(presenter_image, upload_dir, "presenter_")
        presenter_image_sha256 = compute_sha256_for_path(upload_dir / presenter_fname)

    # ── Save presenter video ──────────────────────────────────────────────────
    presenter_video_fname: Optional[str] = None
    presenter_video_sha256: Optional[str] = None
    if presenter_video:
        if presenter_video.content_type not in ALLOWED_VIDEO_TYPES:
            raise HTTPException(status_code=400, detail=f"Invalid presenter video type: {presenter_video.content_type}")
        presenter_video_fname = await _save_upload(presenter_video, upload_dir, "presvid_")
        presenter_video_sha256 = compute_sha256_for_path(upload_dir / presenter_video_fname)

    # ── rights_manifest.json validation ──────────────────────────────────────
    compliance_payload: Optional[dict[str, Any]] = None
    has_real_presenter = bool(presenter_fname or presenter_video_fname)
    if has_real_presenter:
        if not rights_manifest:
            raise HTTPException(status_code=400, detail="使用真人数字人素材时，必须上传 rights_manifest.json")
        if (rights_manifest.filename or "").lower().split(".")[-1] != "json":
            raise HTTPException(status_code=400, detail="rights_manifest 必须是 .json 文件")
        rights_manifest_name = await _save_upload(rights_manifest, upload_dir, "rights_")
        rights_manifest_bytes = (upload_dir / rights_manifest_name).read_bytes()
        try:
            manifest_original = parse_manifest_bytes(rights_manifest_bytes)
            validation_result = validate_rights_manifest(
                manifest=manifest_original,
                selected_language=language,
                selected_platform=platform,
                has_presenter_image=bool(presenter_fname),
                has_presenter_video=bool(presenter_video_fname),
                presenter_image_sha256=presenter_image_sha256,
                presenter_video_sha256=presenter_video_sha256,
        ...
        compliance_payload = _build_compliance_payload(
            validation_result=validation_result,
            manifest_original=manifest_original,
            rights_manifest_filename=rights_manifest_name,
            presenter_image_sha256=presenter_image_sha256,
            presenter_video_sha256=presenter_video_sha256,
        )
    # ── Create VideoTask ──────────────────────────────────────────────────────
    task = VideoTask(
        ...
        voice_preset=voice_preset or language,
        voice_id=voice_id,
        platform=platform,
        ...
        use_bgm=use_bgm,
        qa_report_json={"compliance": compliance_payload} if compliance_payload else None,
    )
    db.add(task)
