...
class ConnectionManager:
    ...
    def disconnect(self, task_id: str, websocket: WebSocket) -> None:
        ...
        logger.debug(f"WS disconnected: task={task_id}")
    ...
    async def broadcast(self, task_id: str, data: dict) -> None:
        ...
        for ws in list(self._connections[task_id]):
            try:
                ...
                logger.debug(f"WS send failed (task={task_id}): {exc}")
                dead.append(ws)
...
async def ws_progress(
    ...
    if token is None:
        ...
        logger.debug(f"WS auth via message: task={task_id}")
    ...
    try:
        ...
        logger.debug(f"WS client disconnected: task={task_id}")
    except Exception as exc:
