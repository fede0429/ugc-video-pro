...
FIXED in v4: Added `stage` field to WS broadcasts so frontend pipeline
stages update correctly. Fixed field names (progress_segment/progress_total).
Fixed WebSocket keep-alive loop that was dropping after 60s.
"""
...
# ── Mapping from orchestrator status_callback keys to frontend pipeline stage keys ──────────
# The frontend PIPELINE_STAGES expects these keys:
#   analyzing_image, extracting_url, generating_script, generating_video, generating_tts, lipsync, stitching
STATUS_KEY_TO_STAGE = {
    "director_planning": "analyzing_image",   # Director planning maps to first visible stage
    "analyzing_image":   "analyzing_image",
    "extracting_url":    "extracting_url",
    "generating_script": "generating_script",
    "generating_video":  "generating_video",
    "polling":           "generating_video",   # Polling for video result = still in video gen stage
    "generating_tts":    "generating_tts",
    "stitching":         "stitching",
    "uploading_drive":   "stitching",         # Drive upload is part of final stage
    "error_drive_upload": "stitching",
}
...
async def run_video_generation(
    ...
    # Broadcast initial status with stage info
    ...
    # Track current stage for progress_callback context
    _current_stage = "generating_video"
    ...
    async def progress_callback(
        ...
        logger.info(f"[task={task_id}] Progress {segment}/{total}")
        ...
        # Broadcast to WebSocket clients — FIXED: include stage + correct field names
        await ws_manager.broadcast(
            task_id,
            {
                "task_id": task_id,
                ...
                "stage_message": f"片段 {segment}/{total} 生成中 ({model}) — 预估 {eta}",
    ...
    async def status_callback(msg_key: str, **kwargs) -> None:
        ...
        logger.info(f"[task={task_id}] Status update: {msg_key} kwargs={kwargs}")

        nonlocal _current_stage

        # Map the orchestrator's status key to a frontend pipeline stage
        stage = STATUS_KEY_TO_STAGE.get(msg_key, _current_stage)
    ...
    await ws_manager.broadcast(
        task_id,
        {
            ...
            "progress_segment": video_result.num_segments,
            "progress_total": video_result.num_segments,
