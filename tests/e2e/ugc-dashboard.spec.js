  ...
  items: [
    {
      ...
      qa_report_json: {
        ...
        compliance: {
          status: 'approved',
          compliance_token: 'cmp_demo123456',
          summary: {
            valid_until: '2027-03-01',
            bound_assets: {
              presenter_image_sha256: 'abc123def4567890',
              presenter_video_sha256: 'fed456abc1237890'
            }
          }
        },
        publish_preparation: {
...
test('上传授权 JSON 并展示摘要', async ({ page }) => {
  await installMocks(page);
  await page.goto('http://127.0.0.1:8000/index.html');
  await page.evaluate(() => {
    localStorage.setItem('ugcvp_access_token', 'demo-token');
    localStorage.setItem('ugcvp_refresh_token', 'refresh-demo');
  });
  await page.reload();
  ...
  const manifest = {
    ...
    rights_owner: '张三',
    authorization_basis: 'signed_release',
  ...
  await page.setInputFiles('#rights-input', {
    name: 'rights_manifest.json',
    mimeType: 'application/json',
    buffer: Buffer.from(JSON.stringify(manifest), 'utf-8'),
  });

  await expect(page.locator('#rights-summary')).toContainText('授权主体：张三');
  await expect(page.locator('#rights-summary')).toContainText('语言：zh / it / en');
});
