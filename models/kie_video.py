...
#   - Kling 2.x i2v: "kling/v2-1-master-image-to-video" (singular image_url)
...
#   - Veo 3.1:    "veo3_fast" / "veo3" (quality) (via /api/v1/veo/*)
...
class KieVideoAdapter(VideoModelAdapter):
    ...
    # ALL models go through CDN upload — KIE.AI rejects base64 data URLs
    # for Veo, Runway, Seedance, and Sora too (not just Kling/Hailuo).
    # CDN upload is free and works universally, so we use it for everything.
    _ALWAYS_USE_CDN = True
    ...
    async def _resolve_image(self, reference_image: str) -> Optional[str]:
        """
        Resolve a reference image to an HTTPS URL suitable for KIE.AI.
        ...
        3. If local file → upload to KIE CDN (ALL models need HTTPS URLs)

        Note: Our server (46.225.212.66:8001) is NOT reachable from KIE.AI.
        Any http://46.225.212.66 URL must be re-uploaded to KIE CDN.
        KIE.AI rejects base64 data URLs for ALL providers, so CDN upload is
        always required regardless of model.
        """
        ...
        # ALL models: upload to KIE CDN (free, universal)
        # KIE.AI rejects base64 data URLs for ALL providers
    ...
    async def _build_input_generic(
        ...
        """Generic endpoint (Kling legacy, etc.).
    ...
    async def _build_input_seedance(
        self,
        prompt: str,
        duration: int,
        reference_image: Optional[str] = None,
        ...
        """Seedance 1.5 / 2.0: model="bytedance/seedance-*"
        Uses generic endpoint but with strict duration constraints.
        Seedance 1.5: duration must be 4, 8, or 12 seconds.
        Seedance 2.0: duration must be 4, 5, 8, 10, or 12 seconds.
        """
        # Snap to nearest valid duration for this model
        valid_durations = self._model_config.get("valid_durations", [4, 8, 12])
        actual_duration = min(valid_durations, key=lambda d: abs(d - duration))
        if actual_duration != duration:
            logger.info(
                f"Seedance duration snap: {duration}s -> {actual_duration}s "
                f"(valid: {valid_durations})"
            )
        ...
        # Seedance supports audio generation
        input_params["generate_audio"] = True
    ...
    async def generate(
        ...
        builder = {
            ...
            "seedance": self._build_input_seedance,
